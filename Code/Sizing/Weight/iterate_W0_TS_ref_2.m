function [ac, flag] = iterate_W0_TS_ref_2(ac, mission_fun, T0, S)
...
function [R, ac] = W0_residual(ac, mission_fun, T0, S, W0)
    ...
    if isequal(mission_fun, @a2a_ref_Ffrac)        
        ...
        dWeng = get_Weng(T0) - get_Weng(ac.initial.T_max);
        % ac.initial.T_mil = ac.initial.T_mil .* (T0 / ac.initial.T_max);
        % ac.initial.T_max = T0;
        
        [Ffrac, ~, ~] = mission_fun(ac);
        ac.a2a.Wf = W0 .* Ffrac;
        ac.a2a.We = empty_weight(ac, mission_fun);
        ac.a2a.We = ac.a2a.We + dWeng;
