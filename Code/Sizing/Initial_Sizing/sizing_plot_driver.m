...
WS_a2a_max_g = max_g(ac.a2a.max_g, ac.a2a.max_g_V, ac.a2a.h_combat, p_half.CLmax, ac.a2a.Wfracs(6));
...
Tfrac = get_thrust_frac(0, 0, 1.08, true, true);
TW_a2a_takeoff = takeoff(WS, 2000, 0, p_half_gear.CD0, p_half_gear.CLmax, 0.8, 0.025, ac.a2a.Wfracs(1), Tfrac);
% Landing
WS_a2a_landing = landing(2000, 0, p_full_gear.CLmax, Wfrac_land_a2a);

% Catapult launch
Tfrac = get_thrust_frac(0, 0, 1.08, true, true);
[WS_a2a_catapult, TW_a2a_catapult] = catapult(WS, TW, ac.a2a.W0, 0.1, K_full_gear, p_full_gear.CLmax, ac.a2a.Wfracs(1), Tfrac);
% Recovery
...
% Vertical load factor
WS_strike_max_g = max_g(ac.strike.max_g, ac.strike.max_g_V, ac.strike.h_combat, p_half.CLmax, ac.strike.Wfracs(6));
...
TW_strike_climb_2 = climb_rate(WS, ac.initial.climb_rate, ac.strike.V_dash, 0, p_clean.CD0, K_clean, false, false, ac.initial.num_eng, false, ac.strike.Wfracs(6), Tfrac);
% Climb 3
Tfrac = get_thrust_frac(0, 0, 1.08, false, false);
...
% Takeoff % TODO set ground roll distance, BPR, mu, 
Tfrac = get_thrust_frac(0, 0, 1.08, true, true);
TW_strike_takeoff = takeoff(WS, 2000, 0, p_half_gear.CD0, p_half_gear.CLmax, 0.8, 0.025, ac.strike.Wfracs(1), Tfrac);
% Landing
WS_strike_landing = landing(2000, 0, p_full_gear.CLmax, Wfrac_land_strike);

% Catapult launch
Tfrac = get_thrust_frac(0, 0, 1.08, true, true);
[WS_strike_catapult, TW_strike_catapult] = catapult(WS, TW, ac.strike.W0, 0.1, K_full_gear, p_full_gear.CLmax, ac.strike.Wfracs(1), Tfrac);
% Recovery
...
figure(1);
clf;
p1 = plot(WS, TW_a2a_dash, "--r");
hold on;
p2 = plot(WS, TW_a2a_turn_rate, "--", "color", "#F9A603");
p3 = plot(WS_a2a_max_g.*ones(n, 1), TW, "-", "color", "#808080");

p4 = plot(WS, TW_a2a_cruise_1, "-", "color", "#0000FF");
p5 = plot(WS, TW_a2a_cruise_2, "-", "color", "#6BADCE");
p6 = plot(WS, TW_a2a_ceiling, "--", "color", "#734F96");

p7 = plot(WS, TW_a2a_climb_to, "--", "color", "#1A2421");
p8 = plot(WS, TW_a2a_climb_ap, "--", "color", "#0B6623");
p9 = plot(WS, TW_a2a_climb_1, "-", "color", "#028A0F");
p10 = plot(WS, TW_a2a_climb_2, "-", "color", "#028A0F");

p11 = plot(WS, TW_a2a_takeoff, "--", "color", "#222021");
...
scatter(4000, ac.initial.T_max./ac.a2a.W0, 100, [252, 106, 3]./255, "filled");
scatter(4000, ac.initial.T_mil./ac.a2a.W0, 100, "black", "filled");

ylim([0, 1.5]);
xlim([0, 10000]);

label(p1, "Dash", 'location', 'right', 'interpreter', 'latex', 'slope');
label(p2, "Sustained Turn", 'location', 'right', 'interpreter', 'latex', 'slope');
label(p3, "Vertical Load Factor", 'location', 'center', 'interpreter', 'latex', 'slope');
label(p4, "Cruise 1", 'location', 'right', 'interpreter', 'latex', 'slope');
label(p5, "Cruise 2", 'location', 'right', 'interpreter', 'latex', 'slope');
label(p6, "Ceiling", 'location', 'right', 'interpreter', 'latex', 'slope');
label(p7, "SEROC Takeoff", 'location', 'right', 'interpreter', 'latex', 'slope');
label(p8, "SEROC Approach", 'location', 'right', 'interpreter', 'latex', 'slope');
label(p9, "Climb 1", 'location', 'right', 'interpreter', 'latex', 'slope');
label(p10, "Climb 2", 'location', 'right', 'interpreter', 'latex', 'slope');
label(p11, "Takeoff", 'location', 'right', 'interpreter', 'latex', 'slope');
label(p12, "Landing", 'location', 'center', 'interpreter', 'latex', 'slope');
label(p13, "Catapult", 'location', 'center', 'interpreter', 'latex', 'slope');
label(p14, "Recovery", 'location', 'center', 'interpreter', 'latex', 'slope');

%% Plot Strike
figure(2);
clf;
p1 = plot(WS, TW_strike_dash, "-r");
hold on;
p2 = plot(WS_strike_max_g.*ones(n, 1), TW, "-", "color", "#808080");

p3 = plot(WS, TW_strike_cruise_1, "-", "color", "#0000FF");
p4 = plot(WS, TW_strike_cruise_2, "-", "color", "#6BADCE");
p5 = plot(WS, TW_strike_ceiling, "--", "color", "#734F96");

p6 = plot(WS, TW_strike_climb_to, "--", "color", "#1A2421");
p7 = plot(WS, TW_strike_climb_ap, "--", "color", "#0B6623");
p8 = plot(WS, TW_strike_climb_1, "-", "color", "#028A0F");
p9 = plot(WS, TW_strike_climb_2, "-", "color", "#028A0F");
p10 = plot(WS, TW_strike_climb_3, "-", "color", "#028A0F");

p11 = plot(WS, TW_strike_takeoff, "--", "color", "#222021");
...
label(p1, "Dash", 'location', 'right', 'interpreter', 'latex', 'slope');
label(p2, "Vertical Load Factor", 'location', 'center', 'interpreter', 'latex', 'slope');
% label(p3, "Vertical Load Factor", 'location', 'center', 'interpreter', 'latex', 'slope');
label(p3, "Cruise 1", 'location', 'right', 'interpreter', 'latex', 'slope');
label(p4, "Cruise 2", 'location', 'right', 'interpreter', 'latex', 'slope');
label(p5, "Ceiling", 'location', 'right', 'interpreter', 'latex', 'slope');
label(p6, "SEROC Takeoff", 'location', 'right', 'interpreter', 'latex', 'slope');
label(p7, "SEROC Approach", 'location', 'right', 'interpreter', 'latex', 'slope');
label(p8, "Climb 1", 'location', 'right', 'interpreter', 'latex', 'slope');
label(p9, "Climb 2", 'location', 'right', 'interpreter', 'latex', 'slope');
label(p10, "Climb 3", 'location', 'right', 'interpreter', 'latex', 'slope');
label(p11, "Takeoff", 'location', 'right', 'interpreter', 'latex', 'slope');
label(p12, "Landing", 'location', 'center', 'interpreter', 'latex', 'slope');
label(p13, "Catapult", 'location', 'center', 'interpreter', 'latex', 'slope');
label(p14, "Recovery", 'location', 'center', 'interpreter', 'latex', 'slope');
