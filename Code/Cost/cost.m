...
ac_W_engine = ac_num_engines * 2450;  % F414-GE-400 total weight (lb)
...
cst.aux.labor_rate = 30152.76;  % Labor rate (USD/hr)
cst.aux.price_aircraft = 0;  % Aircraft price (USD) TODO
cst.aux.price_engine = ac_num_engines * 5606000;  % F414-GE-400 total price (USD)
...
cst.unit.N_program = 500 + cst.unit.N_rdte;  % Number of units produced (AIAA RFP). Added RDTE airplanes via Roskam VIII Eq. 4.1
...
cst.unit.C_man_m = cst.unit.MHR_man_program * cst.unit.R_m_m - cst.unit.MHR_man_r * cst.unit.R_m_r;  % Manufacturing labor cost (USD), Roskam VIII Eq. 4.10a
...
% Tooling Cost
cst.unit.N_r_m = 15;  % Production rate (units/month), source progress report from F-18A
...
cst.unit.C_tool_m = cst.unit.MHR_tool_program * cst.unit.R_t_m - cst.unit.MHR_tool_r * cst.unit.R_t_r;  % Tooling cost (USD), Roskam VIII Eq. 4.14a

% Quality Control Cost
cst.unit.C_qc_m = 0.13 * cst.unit.C_man_m;  % Quality control cost (USD), Roskam VIII Eq. 4.16

% Totals
cst.unit.program_production_cost = cst.unit.C_man_m + cst.unit.C_mat_m + cst.unit.C_tool_m + cst.unit.C_qc_m;  % Total program production cost (USD), Roskam VIII 

cst.unit.airplane_price = cst.unit.program_production_cost / cst.unit.N_program;  % Unit cost (USD)

%% Environmental, Fleet, Compatibility, Weapons
cst.EFCW.AIM120_price = 916000;  % Cost per AIM-120 (USD)
cst.EFCW.AIM9X_price = 500000;  % Cost per AIM-9X (USD)
cst.EFCW.JDAM_price = 22000 + 3026.54;  % Cost per JDAM (USD)
cst.EFCW.life_support = 0;  % Life support cost (USD) TODO
cst.EFCW.avionics = 0;  % Avionics cost (USD) TODO
cst.EFCW.surface_treat = 0;  % Surface treatment cost (USD, or USD/gal) TODO
cst.EFCW.sub_tech = 0;  % Subsystem technologies cost (USD)

%% COC
cst.COC.crew_ratio = 1.1 * ac_num_crew;  % Number of pilots per aircraft (Raymer 18.5.1)
cst.COC.pilot_rate = 162.16;  % Pilot hourly rate (USD)
...
cst.COC.fuel_cost = 1.02 * ac.initial.Wf * (cst.aux.fuel_price / cst.aux.fuel_density);  % Fuel cost per mission (USD)
...
cst.COC.W_oil = 0.0125 * ac.initial.Wf * (cst.aux.block_time_avg / 100);  % Oil weight per mission (lb)
